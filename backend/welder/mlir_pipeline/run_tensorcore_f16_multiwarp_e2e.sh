...
ART_DIR="${ROOT_DIR}/mlir_pipeline/tensorcore_f16_multiwarp_artifacts"
mkdir -p "${ART_DIR}"

echo "[1/2] compile to nvvm (tensorcore f16, multi-warp) -> ${ART_DIR}"
OUT_DIR="${ART_DIR}" \
  bash "${ROOT_DIR}/compiler/run_welder_to_nvvm_isa.sh" \
  "${ROOT_DIR}/mlir_pipeline/matmul_host_shared_f16.mlir" \
  --enable-tensorcore-f16 \
  --force-tile-m 32 --force-tile-n 16 --force-tile-k 16
...
echo "PASS: tensorcore f16 multi-warp e2e verified (.maxntid x=${threads})."
