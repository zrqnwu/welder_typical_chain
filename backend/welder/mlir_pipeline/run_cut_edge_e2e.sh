...
# 复用其它脚本的“自动发现 LLVM/MLIR build”逻辑。
...
IN_MLIR="${1:-${SCRIPT_DIR}/cut_edge_diamond.mlir}"
OUT_DIR="${OUT_DIR:-${SCRIPT_DIR}/cut_edge_artifacts}"
mkdir -p "${OUT_DIR}"
...
echo "[1/4] welder-compiler (generic + cut-edges) -> ${AFTER_WELDER}"
...
echo "[4/4] sanity check: expect >= 2 PTX .entry (multi-kernel)"
entry_count="$(grep -c "\\.entry" "${LOWERED_NVVM}" || true)"
if [[ "${entry_count}" -ge 2 ]]; then
  echo "PASS: Found ${entry_count} .entry (>=2)."
else
  echo "FAIL: Found ${entry_count} .entry (expected >=2)."
  echo "hint: inspect ${LOWERED_NVVM} and search for '.entry'." >&2
  exit 1
fi
