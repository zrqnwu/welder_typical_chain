// 用例：MatMul + Bias(Add) + ReLU（f32）- Transformer/MLP FC2 epilogue
//
// 说明：
...
module {
  func.func @main(
      %A: tensor<256x4096xf32>,
      %B: tensor<4096x1024xf32>,
      %Bias: tensor<256x1024xf32>) -> tensor<256x1024xf32> {
    ...
    } ins(%C, %Bias : tensor<256x1024xf32>, tensor<256x1024xf32>)
      outs(%Dinit : tensor<256x1024xf32>) {
    ^bb0(%x: f32, %b: f32, %out: f32):
      %y = arith.addf %x, %b : f32
      linalg.yield %y : f32
    } -> tensor<256x1024xf32>

    %E0 = tensor.empty() : tensor<256x1024xf32>
    %Einit = linalg.fill ins(%c0f : f32) outs(%E0 : tensor<256x1024xf32>)
        -> tensor<256x1024xf32>
    %E = linalg.generic {
      indexing_maps = [
      ...
      iterator_types = ["parallel", "parallel"]
    } ins(%D : tensor<256x1024xf32>) outs(%Einit : tensor<256x1024xf32>) {
    ^bb0(%in: f32, %out: f32):
      %relu = arith.maximumf %in, %c0f : f32
      linalg.yield %relu : f32
    } -> tensor<256x1024xf32>

    return %E : tensor<256x1024xf32>
  }
}
