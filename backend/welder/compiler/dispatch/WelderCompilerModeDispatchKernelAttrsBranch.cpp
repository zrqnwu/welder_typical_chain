...
int buildTransformLibraryFromKernelAttrsBranch(
    const ModeDispatchContext &modeCtx) {
  ...
  (void)enableSoftwarePipelining;
  (void)pipelineDepth;
  (void)pipelinePeelEpilogue;
  (void)enableCutEdges;
  (void)reductionChainSplitBroadcastEdges;
  (void)enableRowReductionChainReuseFusion;
  (void)enableRegisterLevelSchedule;
  (void)traceVerbose;

  auto branchImpl = [&]() -> int {
#include "WelderCompilerModeDispatchKernelAttrsBranchBody.h"
  };
  return branchImpl();
}
