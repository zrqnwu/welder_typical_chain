...
# Keep 性能测量/compilation runs stable and fast: only build the compiler
# target by 默认 instead of rebuilding every target in this CMake tree.
BUILD_TARGET="${WELDER_BUILD_TARGET_COMPILER:-welder-compiler}"
...
COMPILER="${BUILD_DIR}/welder-compiler"
if [[ ! -x "${COMPILER}" ]]; then
  echo "error: welder-compiler not found at: ${COMPILER}" >&2
  exit 2
fi
